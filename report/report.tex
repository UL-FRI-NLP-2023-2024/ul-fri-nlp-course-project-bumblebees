...
Previous approaches, like MLM 
This approach also outperforms methods like MLM which utilized multiple CLS token vector embeddings in favour of producing a single vector embedding. This is because instead of applying CLS pooling on token vectors, TSDAE applies mean pooling that takes the average value across all token vectors to create a sentence vector ~\cite{pineconetsdae}.
