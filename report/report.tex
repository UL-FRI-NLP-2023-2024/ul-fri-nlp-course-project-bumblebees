...
The approaches were tested on the test set and evaluated using the F1 score.
...
TODO
% tu še povemo, da smo testirali 2 različna T5 modela
T5 without Slovene language: {\it msmarco-14langs-mt5-base-v1}.
T5 specifically for Slovene language: {\it slv\_doc2query}.
%------------------------------------------------
...
The base model {\it paraphrase-multilingual-MiniLM-L12-v2}~\cite{reimers-2019-sentence-bert}, base model fine-tuned with the TSDAE method (denoted as TSDAE), base model fine-tuned with the GPL method and {\it msmarco-14langs-mt5-base-v1} for the T5 model (denoted as GPL) and base model fine-tuned with the GPL method and {\it slv\_doc2query} for the T5 model (denoted as $\text{GPL}_{SLO}$) were evaluated on the test set part of SentiNews dataset. The results are presented in Table~\ref{tab2}.
