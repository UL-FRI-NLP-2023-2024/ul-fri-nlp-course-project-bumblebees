...
Interestingly, the GPL model performs better than the $\text{GPL}_{SLO}$ (the opposite happened with {\it paraphrase-multilingual-MiniLM-L12-v2} model in Table~\ref{tab2}), although the difference between the two is only 3.28\% in F1 score. %We are uncertain about the reasoning behind this difference. One idea could be, that since the {\it SloBERTa} model is already doing very well with the Slovene language, it is not influenced as much by the queries provided by the T5 model. Meanwhile with the Slovenian T5, the queries are probably quite good and thus influence the fine-tuning a lot more. Je tulk sigurn za slovenščino, da se pol nit ne sredotoča tulku na querije, aka vpliv T5 modela, ki ni za slovenščino je manjpši. Medtem ku pa so pr slovenskm T5 tud queriji, in nima base model sam po sebi tulk vpliva - vemo pa, da ma base model sam bulše rezultate
...
Comparing the two base models, {\it paraphrase-multilingual-MiniLM-L12-v2}~\cite{reimers-2019-sentence-bert} and {\it SloBERTa}, we noticed that the latter performs better for sentiment classification of Slovenian sentences. The {\it SloBERTa} surpassed the {\it paraphrase-multilingual-MiniLM-L12-v2} F1 score by 4.08\%. This is most probably due to {\it SloBERTa} being trained only on Slovenian datasets, while for the {\it paraphrase-multilingual-MiniLM-L12-v2}, the Slovene language was only a part of the training set.

Unfortunately, the combination $\text{GPL}_{SLO}$+TSDAE could not be evaluated for neither of the base models. Comparing it to the combination TSDAE+$\text{GPL}_{SLO}$, that we were able to fine-tune and test, could prove interesting.
